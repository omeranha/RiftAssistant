...
{
	private readonly Dictionary<uint, Dictionary<uint, Scene>> dictionary_0 = new Dictionary<uint, Dictionary<uint, Scene>>();

	private readonly Dictionary<uint, uint> dictionary_1 = new Dictionary<uint, uint>();

	public List<SceneHint> list_0 = new List<SceneHint>();

	private readonly Allocator class351_0 = new Allocator(8192, "SceneRevealInfoAllocator");

	private readonly AllocationCache<r_SceneRevealInfo> class352_0 = new AllocationCache<r_SceneRevealInfo>(88);

	internal r_Scene[] Buffer_Scenes { get; private set; }

	internal r_World[] Buffer_Worlds { get; private set; }

	public IEnumerable<Scene> method_0(uint uint_0)
	{
	...
	public Scene method_1(uint uint_0, uint uint_1)
	{
	...
	internal bool Collect()
	{
		...
		if (num > 0)
		{
			Buffer_Scenes = GameWindowManager.ReadArray<r_Scene>(CoreCollector.D3Memory.SceneContainer.DataAddress, num);
		...
		if (num > 0)
		{
			Buffer_Worlds = GameWindowManager.ReadArray<r_World>(CoreCollector.D3Memory.WorldContainer.DataAddress, num);
		}
		...
		for (int j = 0; j < Buffer_Scenes.Length; j++)
		{
			...
			SnoArea snoArea = SnoData.Areas.GetSnoArea(r_Scene2.LevelAreaSNO);
			if (snoArea == null)
			{
				continue;
			}
			...
			{
				value = new Dictionary<uint, Scene>();
				dictionary_0.Add(r_Scene2.SWorldID, value);
			}
			SnoScene scene = SnoData.Scenes.GetScene(r_Scene2.SceneSno);
			if (scene != null)
			{
				uint uint_ = method_2(r_Scene2.SWorldID);
				Scene value3 = new Scene(scene, snoArea, uint_, r_Scene2.SSceneID, r_Scene2.Id, r_Scene2.mesh_min_x, r_Scene2.mesh_min_y, r_Scene2.mesh_max_x, r_Scene2.mesh_max_y, r_Scene2.mesh_max_z);
		...
		if (CoreCollector.DAF.LevelAreaAddress != 0L)
		{
			...
			if (class351_0.RawAllocator.ElementSize == 88)
			{
				...
				foreach (r_SceneRevealInfo item in class352_0.GetItems())
				{
					...
					{
						value4 = new Dictionary<uint, Scene>();
						dictionary_0.Add(item.uint_2, value4);
					}
					SnoScene scene2 = SnoData.Scenes.GetScene(item.uint_0);
					if (scene2 != null)
					{
						uint uint_2 = method_2(item.uint_2);
						Scene value6 = new Scene(scene2, null, uint_2, item.uint_1, 0u, item.float_0, item.float_1, item.float_2, item.float_3, 0f);
	...
	internal void GenerateSceneHints()
	{
		...
		foreach (Scene item in method_0(CoreCollector.LocalPlayer.WorldId))
		{
			...
			bool flag = false;
			foreach (Portal item2 in CoreCollector.ActorCollector.Class112_1.IEnumerable_0)
			{
